...
export const adminAPI = {
  ...
  rejectUser: (id: string, reason: string) => api.put(`/admin/users/${id}/reject`, { reason }),
  ...
  getGrowthData: (months?: number) => api.get('/admin/analytics/growth', { params: { months } }),
  getContactLogs: (params?: any) => api.get('/admin/contact-logs', { params }),
  listProfiles: (params?: any) => api.get('/admin/profiles', { params }),
};
...
// Extended Bureau API (add missing methods to bureauAPI above if needed)
export const bureauAPIExtended = {
  getNotifications: () => api.get('/bureau/notifications'),
  markNotificationRead: (id: string) => api.patch(`/bureau/notifications/${id}/read`),
  markAllNotificationsRead: () => api.patch('/bureau/notifications/read-all'),
};

// Merge the missing methods onto bureauAPI
Object.assign(bureauAPI, {
  getNotifications: bureauAPIExtended.getNotifications,
  markNotificationRead: bureauAPIExtended.markNotificationRead,
  markAllNotificationsRead: bureauAPIExtended.markAllNotificationsRead,
});

// Extend profileAPI with getMyProfiles alias
Object.assign(profileAPI, {
  getMyProfiles: (page = 1, limit = 20) => api.get('/profiles/mine', { params: { page, limit } }),
});

// Extend adminAPI with getDashboard
